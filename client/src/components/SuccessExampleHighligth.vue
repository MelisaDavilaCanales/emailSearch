...
const paragraphs = ref([
  "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Iste veniam enim illum a. Impedit voluptatem pariatur consequatur suscipit.",
  "Lorem ipsum dolor sit amet consectetur adipisicing elit. Iste nemo laborum omnis deleniti soluta possimus quod architecto.",
  "Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequatur asperiores minima consequuntur.",
  "Lorem ipsum dolor sit amet consectetur adipisicing elit. Recusandae laudantium dolore repellat unde neque, nesciunt suscipit blanditiis.",
  "lorem100"
]);
...
const highlightText = (text: string, term: string): string => {
  if (!term) return text;
  const regex = new RegExp(`(${term})`, 'gi'); // Busca el término ignorando mayúsculas/minúsculas
  return text.replace(regex, '<span class="highlight">$1</span>'); // Agrega el span con clase "highlight"
};

// Procesa los textos para incluir el resaltado
const highlightedParagraphs = computed(() => {
  return paragraphs.value.map(paragraph =>
    highlightText(paragraph, searchContent.value)
  );
});
...
<template>
  ...
  <!-- Textos con resaltado -->
  <div>
    <p v-for="(paragraph, index) in highlightedParagraphs" :key="index" v-html="paragraph" class="paragraph"></p>
  </div>
</template>
